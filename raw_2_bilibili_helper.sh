...
if [ ! -d "/ql/data/scripts/bilibili/" ]; then
	mkdir /ql/data/scripts/bilibili
fi
cd bilibili
...
download() {
	...
	if [ ! -f "/ql/data/config/config.json" ]; then
		echo "配置文件不存在。"
		cp -f ./tmp/config.json /ql/data/config/config.json
	fi
	echo "清除缓存........."
	rm -rf tmp
...
if [ ! -f "/ql/data/scripts/bilibili/BILIBILI-HELPER.jar" ]; then
	echo "没找到BILIBILI-HELPER.jar，开始下载.........."
	download
fi
files=$(ls /ql/data/config/*.json)
